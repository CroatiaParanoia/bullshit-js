...
function getSentence(list) {
    var tempLocation = Math.floor(Math.random() * list.length);
    return list[tempLocation];
}
console.log(213);
function getRandomValue(mix, max) {
    if (mix === void 0) { mix = 0; }
    if (max === void 0) { max = 100; }
    return Math.random() * (max - mix) + mix;
}
function getQuote() {
    var quote = getSentence(quotes);
    quote = quote.replace("曾经说过", getSentence(prefix));
    quote = quote.replace("这不禁令我深思", getSentence(postfix));
    return quote;
}
function addParagraph(_chapter) {
...
function bullshitGenerator(subject, paragraphLength, chapterLength) {
    if (chapterLength === void 0) { chapterLength = 6000; }
    function getDiscuss() {
        var words = getSentence(discuss);
        return words.replace(/主题/g, subject);
    }
    var article = [];
    // @ts-ignore
    // eslint-disable-next-line guard-for-in
    for (var i = 0; i < (paragraphLength || subject.length); i++) {
        var chapter = "";
        var curChapterLength = 0;
        while (curChapterLength < chapterLength) {
            ...
            if (randomValue < 5 && chapter.length > 200) {
                ...
                var quote = getQuote();
                curChapterLength = chapterLength + quote.length;
                chapter = chapter + quote;
            }
            else {
                var discuss_1 = getDiscuss();
                curChapterLength = chapterLength + discuss_1.length;
                chapter = chapter + discuss_1;
            }
        }
        chapter = addParagraph(chapter);
        article.push(chapter);
...
export { bullshitGenerator as default };
